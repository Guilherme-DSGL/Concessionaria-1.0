...
function Header()
{
    ...
    global $pdo;
    $id = $_GET['id'];
    $sql = $pdo-> prepare('SELECT * from venda where id = :id');
    $sql-> bindValue('id', $id);
    $sql-> execute();
    global $comp;
    $comp = $sql -> fetch();
        ...
        $this->Cell(65,7,utf8_decode('Codigo da compra:').$comp['id'],1,0,'C');
        $this->Ln(7);
        $g=38;
